...
function buyBiomeUpgrade(biomeId, def) {
  const prog = biomeData(biomeId); const lvl = prog.upgrades[def.id] || 0; if (lvl >= def.max) return; const cost = def.base + def.step * lvl; if (def.type === 'token') { if (!spendBiomeTokens(biomeId, cost)) { toast('Nincs elég ebből a biom tokenből.', 'bad'); return; } } else { if (state.money < cost) { toast('Nincs elég pénz.', 'bad'); return; } state.money -= cost; }
...
function queueReveal(turtle) {
  if (state.globalUpgrades.revealSkip) { state.turtles.push(turtle); addCatchLog(turtle, false); toast(`${speciesById(turtle.speciesId).name} a boardra került.`); if (!state.stats.firstBoard) { state.stats.firstBoard = true; setHint('firstBoard'); } rerender(); return; }
...
function sellTurtle(uid, e = null) {
  const i = state.turtles.findIndex(x => x.uid === uid); if (i === -1) return; const t = state.turtles[i]; const value = turtleValue(t); if (value <= 0) { toast('A tojás nem eladható.', 'bad'); return; } state.money += value; state.turtles.splice(i, 1); state.selectedTurtles = state.selectedTurtles.filter(x => x !== uid); toast(`Eladva: +${fmt(value)} pénz`); if (e) { floater(e.clientX, e.clientY, `+${fmt(value)}`, 'money'); }
  if (state.money >= 200 && !state.stats.first200) { state.stats.first200 = true; setHint('first200'); }
  rerender();
}
function toggleSelect(uid, preferBreed = false) {
  const t = state.turtles.find(x => x.uid === uid); if (!t) return; if (state.selectedTurtles.includes(uid)) state.selectedTurtles = state.selectedTurtles.filter(x => x !== uid); else state.selectedTurtles = [...state.selectedTurtles.slice(-1), uid];
  if (preferBreed && state.selectedTurtles.length === 2) tryBreedSelected(); else rerender();
}
function tryBreedSelected() {
  if (state.selectedTurtles.length < 2) { toast('Válassz ki két teknőst.', 'bad'); return; } const [aId, bId] = state.selectedTurtles; if (aId === bId) { toast('Két külön teknős kell.', 'bad'); return; } const a = state.turtles.find(t => t.uid === aId), b = state.turtles.find(t => t.uid === bId); if (!a || !b) { state.selectedTurtles = []; rerender(); return; } if (!canBreed(a) || !canBreed(b)) { toast('Csak kész, felnőtt teknősök párosíthatók.', 'bad'); return; }
